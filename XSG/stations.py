...
class Station():
    ...
    def __init__(
        ...
        self.inventory = [initial_values] + [0] * (game.weeks-1)
    ...
    def add_supplier(self,supplier):
        ...
        for i in range(self.delay_shipping):
            self.queue_receive[supplier.station_name].put(self.initial_values)
            self.outstanding_orders_to_suppliers[supplier.station_name][0] += self.initial_values
        for i in range(self.delay_ordering):
            self.queue_transmit[supplier.station_name].put(self.initial_values)
            self.outstanding_orders_to_suppliers[supplier.station_name][0] += self.initial_values
    ...
    def check_endnode(self):
        if len(self.suppliers) == 0:
            ...
            for i in range(self.delay_shipping):
                self.queue_receive['endnode'].put(self.initial_values)
                self.outstanding_orders_to_suppliers['endnode'][0] += self.initial_values
    ...
    def decide_shipment(self,week):
        ...
        if backorders > 0:
            ...
            for k,v in self.backorder.items():
                shipment[k] = floor(total_shipment/self.n_customers)  # ship more to customers with more backorders
    ...
    def process(self,week):
        ...
        for x in supplier_names:
            ...
            if week == 0:
                self.outstanding_orders_to_suppliers[x][0] -= self.inbound[x][0]
            else:
                self.outstanding_orders_to_suppliers[x][week] = self.outstanding_orders_to_suppliers[x][week-1] - self.inbound[x][week]

        # adjust inventory, calculate backorder, decide shipment
        if week == 0:
            self.inventory[0] += week_sum(self.inbound,week)
        else:
            self.inventory[week] = self.inventory[week-1] + week_sum(self.inbound,week)
        ...
        # decide on PO
        order = self.decide_order(week)  # player order decision goes here
        for x in supplier_names:
            if week == 0:
                self.outstanding_orders_to_suppliers[x][0] += order[x]
            else:
                self.outstanding_orders_to_suppliers[x][week] = self.outstanding_orders_to_suppliers[x][week-1] + order[x]
        if supplier_names is ['endnode']:
